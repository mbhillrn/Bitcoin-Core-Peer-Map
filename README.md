...
MBCore Dashboard uses Bitcoin Core's built in tools to query your running node, and turns it into a living network map. It quieries using Bitcoin Core's built-in tools, geolocates peers via a maintained database and online geolocation services like [ip-api.com](http://ip-api.com), then aggregates them by Autonomous System (AS) to show how your connections are distrobuted across ISPs and hosting providers. All peer location data (IP, latitude, longitude, ISP, AS, and more) is stored in a local SQLite database for instant recall and is continuously updated from the  [Bitcoin Node GeoIP Dataset](https://github.com/mbhillrn/Bitcoin-Node-GeoIP-Dataset).
